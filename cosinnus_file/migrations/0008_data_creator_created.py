...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for entry in orm.FileEntry.objects.all():
            entry.creator = entry.uploaded_by
            entry.created = entry.uploaded_date
            entry.save()

    def backwards(self, orm):
        for entry in orm.FileEntry.objects.all():
            entry.uploaded_by = entry.creator
            entry.uploaded_date = entry.created
            entry.save()
